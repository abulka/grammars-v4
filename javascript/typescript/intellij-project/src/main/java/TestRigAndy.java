...
public class TestRigAndy {
    ...
    public TestRigAndy(String[] args) throws Exception {
        if (args.length < 2) {
    ...
    public static void main(String[] args) throws Exception {
        org.antlr.v4.gui.TestRig testRig = new org.antlr.v4.gui.TestRig(args);
        if (args.length >= 2) {
            testRig.process();
        }
...
//        ANDY DEBUG
        System.out.println("TestRigAndy.process()");
        System.out.println("lexerName = " + lexerName);
        System.out.println("cl = " + cl);
        System.out.println("lexerClass = " + lexerClass);
//      print  grammarName
        System.out.println("grammarName = " + grammarName);
//      print startRuleName
        System.out.println("startRuleName = " + startRuleName);
//      print inputFiles
        System.out.println("inputFiles = " + inputFiles);
//      print printTree
        System.out.println("printTree = " + printTree);
        ...
        Lexer lexer = (Lexer)lexerCtor.newInstance((CharStream)null);
        ...
        if (this.inputFiles.size() == 0) {
            ...
            for(Iterator var15 = this.inputFiles.iterator(); var15.hasNext(); this.process(lexer, parserClass, parser, charStream)) {
                String inputFile = (String)var15.next();
    ...
    protected void process(Lexer lexer, Class<? extends Parser> parserClass, Parser parser, CharStream input) throws IOException, IllegalAccessException, InvocationTargetException, PrintException {
        ...
        tokens.fill();
        if (this.showTokens) {
            Iterator var6 = tokens.getTokens().iterator();

            while(var6.hasNext()) {
                Token tok = (Token)var6.next();
